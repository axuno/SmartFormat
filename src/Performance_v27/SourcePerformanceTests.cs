...
* Legends *
  ...
  1 us      : 1 Microsecond (0.000001 sec)
    ...
    public class SourcePerformanceTests
    {
        ...
        private readonly IOutput _stringOutput;

        public SourcePerformanceTests()
        {
            //_nullOutput = new NullOutput();
            _stringOutput = new StringOutput();
        ...
        public void Setup()
        {
            _pureParsingParser.UseAlternativeEscapeChar();
            _pureParsingParser.AddAlphanumericSelectors();
            _pureParsingParser.AddAdditionalSelectorChars("_-");
            _pureParsingParser.AddOperators(".?,[]");
        }
        ...
        public void SfWithLiterals()
        {
            for (var i = 0; i < N; i++)
            {
                 _literalFormatter.FormatWithCacheInto(ref _formatCacheLiteral, _stringOutput, "Address: {0} {1}, {2}\nName: {3} {4}", _reflectionAddress.City.ZipCode,
                    _reflectionAddress.City.Name, _reflectionAddress.City.AreaCode,
                    _reflectionAddress.Person.FirstName, _reflectionAddress.Person.LastName);
            }
        }
        ...
        public void SfCacheReflection()
        {
            for (var i = 0; i < N; i++)
            {
                _reflectionFormatter.FormatWithCacheInto(ref _formatCache, _stringOutput, "", _reflectionAddress);
            }
        }
        ...
        public void SfWithDictionary()
        {
            for (var i = 0; i < N; i++)
            {
                _dictionaryFormatter.FormatWithCacheInto(ref _formatCache, _stringOutput, "", _dictionaryAddress);
            }
        }
        ...
        public void SfWithJson()
        {
            for (var i = 0; i < N; i++)
            {
                _jsonFormatter.FormatWithCacheInto(ref _formatCache, _stringOutput, "", _jsonAddress);
            }
        }
